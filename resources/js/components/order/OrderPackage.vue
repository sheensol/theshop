<template>
    <div>
        <div class="grey lighten-4 border border-gray-200 pa-4 rounded d-flex justify-space-between align-center" v-if="is_addon_activated('multi_vendor')">
                            ...
                            && (!order.has_refund_request)
                                && order.payment_status == 'paid'
                                    && today <= order.created_at + refundSettings.refund_request_time_period
                                        && refundSettings.refund_request_order_status.includes(order.delivery_status)" @click="refundRequest(order.id)"
                >{{ $t('request_refund') }}
                </div>
            </span>
        </div>
        ...
        <div class="grey lighten-4 pa-4" v-if="order.courier_name">
            ...
            <v-row class="border-top border-gray-300 border-end" no-gutters>
                ...
                <v-col cols="12" md="4" class="border-bottom border-start border-gray-300">
                    ...
                    <div class="pa-3"><a :href="order.tracking_url" target="_blank">{{ $t('track') }}</a></div>
                </v-col>
            </v-row>
        </div>
            ...
            <v-col xl="5" md="6" cols="12" order="1" order-md="2">
                <v-list dense>
                    ...
                    <v-list-item class="fw-700">
                        ...
                        <v-list-item-content class="align-end col-4 justify-end">{{ format_price(order.coupon_discount) }}</v-list-item-content>
                    </v-list-item>
                </v-list>
                <div class="grey lighten-4 border border-gray-200 rounded">
                    <v-list-item class="fw-700">
                        <v-list-item-content>{{ $t('total') }} :</v-list-item-content>
                        <v-list-item-content class="align-end col-4 justify-end px-0">{{ format_price(order.grand_total) }}</v-list-item-content>
                    </v-list-item>
                </div>
            </v-col>
        </v-row>
