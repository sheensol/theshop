<template>
    <div>
        <banner :loading="false" :banner="$store.getters['app/banners'].all_shops_page" class="mb-5"/>
    ...
    methods:{
        async getBrands(){
            ...
            if (res.data.success) {
                ...
                this.selectedBrandId = this.allBrands.some(brand => brand.id == this.$route.query.brandId )
                    ? this.allBrands.find(brand => brand.id == this.$route.query.brandId ).id
                    : null
            }
        },
        async getCategories(){
            ...
            if (res.data.success) {
                ...
                this.selectedCategoryId =this.allCategories.some(category => category.id == this.$route.query.categoryId )
                    ? this.allCategories.find(category => category.id == this.$route.query.categoryId ).id
                    : null
            }
        },
        categoryChange(id){
            ...
            this.$router.push({ query: { ...this.$route.query, categoryId: this.queryParam.categoryId, page: this.queryParam.page } }).catch(() => {})
            this.getShops({ categoryId: id })
        },
        brandChange(id){
            this.queryParam.brandId = id;
            this.$router.push({ query: { ...this.$route.query, brandId: this.queryParam.brandId, page: this.queryParam.page } }).catch(() => {})
            this.getShops({ brandId: id })
        },
        pageSwitch(pageNumber){
            this.$router.push({ query: { ...this.$route.query, page: this.queryParam.page } }).catch(() => {})
