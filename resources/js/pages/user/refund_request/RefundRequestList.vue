        ...
        <div>
            ...
            >
                ...
                <template v-slot:[`item.products`]="{ item }">
                    <div v-for="(refunditem, i) in item.refunditems" :key="i" class="mb-3">
                        <div class="text-truncate-2">{{ refunditem.product.name }}</div>
                        <div class="" v-if="refunditem.product.combinations.length > 0">
                            <span v-for="(combination, j) in  refunditem.product.combinations" :key="j" class="me-4 py-1 fs-12">
                ...
                <template v-slot:[`item.status`]="{ item }">
                    <v-btn v-if="item.status == 0" x-small color="info" elevation="0">{{ $t('pending') }}</v-btn>
                    <v-btn v-else-if="item.status == 1" x-small color="success"  elevation="0">{{ $t('accepted') }}</v-btn>
                    <v-btn v-else="item.status == 2" x-small color="error"  elevation="0">{{ $t('rejected') }}</v-btn>
                </template>
            </v-data-table>
